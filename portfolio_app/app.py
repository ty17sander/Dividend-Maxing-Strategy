...
def compute_allocation(scores, cfg):
    """
    Select the best funds and size them to maximise income + return.

    Reads directly from cfg so every slider change takes effect immediately:
      - cfg["max_yieldmax_pct"]  — YieldMax family cap (e.g. 0.60 = 60%)
      - cfg["max_position_pct"]  — single position cap (e.g. 0.15 = 15%)
      - cfg["max_positions"]     — max number of holdings (default 12)

    Selection: top N YieldMax + top M non-YieldMax as a diversification buffer.
    The split between YM and non-YM is driven by max_yieldmax_pct — if user
    sets it to 30%, fewer YieldMax funds are selected and more non-YM fill in.
    Weighting: total_return^1.5 so the strongest performers get the most capital.
    """
    max_pos    = cfg.get("max_position_pct", 0.15)
    max_ym_pct = cfg.get("max_yieldmax_pct", 0.60)   # ← reads the slider
    n_total    = cfg.get("max_positions", 12)

    # Work out how many YieldMax slots to give based on the cap
    # If cap is 60% and we have 12 positions at equal weight (8.3% each),
    # at most 7 YieldMax fit within 60%. Use that as the slot count.
    # But also always keep at least 2 non-YM for diversification.
    max_ym_slots = max(int(max_ym_pct / max_pos), 1)   # e.g. 0.60/0.15 = 4 slots minimum
    max_ym_slots = min(max_ym_slots, n_total - 2)        # always leave 2 non-YM slots
    n_non_ym     = n_total - max_ym_slots

    # Only consider tickers that are in scores (have real data)
    scored = set(scores.keys())

    ym_ranked = sorted(
        [t for t in ELIGIBLE if ELIGIBLE[t]["cat"] == "YieldMax" and t in scored],
    ...
    # Weight by total_return^1.5 — concentrate in actual winners
    weights_raw = {t: max(scores[t]["total_ret"], 0.01) ** 1.5 for t in selected}
    tot = sum(weights_raw.values())
    w   = {t: v / tot for t, v in weights_raw.items()}

    # Enforce per-position cap and YieldMax % cap together
    for _ in range(30):
        # Per-position cap
        excess = 0.0
        capped = {}
        ...
        # YieldMax family % cap
        ym_total = sum(wt for t, wt in capped.items() if ELIGIBLE[t]["cat"] == "YieldMax")
        if ym_total > max_ym_pct:
            scale = max_ym_pct / ym_total
            for t in list(capped):
                if ELIGIBLE[t]["cat"] == "YieldMax":
                    excess += capped[t] * (1 - scale)
                    capped[t] *= scale
        ...
        # Redistribute excess to positions below cap
        under = [t for t, wt in capped.items() if wt < max_pos]
        if not under:
            w = capped
            break
        add = excess / len(under)
        for t in under:
            capped[t] = min(capped[t] + add, max_pos)
        w = capped
